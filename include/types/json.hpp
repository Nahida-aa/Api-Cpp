...
class Json {
public:
    enum class Type { Null, Boolean, Number, String, Array, Object }; //表示 JSON 对象的类型

    using ArrayType = std::vector<Json>; //表示 JSON 对象的数组类型
    using ObjectType = std::unordered_map<std::string, Json>; // 表示 JSON 对象的对象类型

private:
    std::variant<std::nullptr_t, bool, double, std::string, ArrayType, ObjectType> value_; // 使用 std::variant 存储 JSON 对象的值，可以是 null、布尔值、数字、字符串、数组或对象
...
public:
    ...
    Json(std::initializer_list<std::pair<const std::string, Json>> init) : value_(ObjectType(init)) {}
    ...
    size_t size() const { // 返回 JSON 对象的大小
        if (is_array()) return std::get<ArrayType>(value_).size();
        else if (is_object()) return std::get<ObjectType>(value_).size();
        return 1;
    }
    bool empty() const { // 返回 JSON 对象是否为空
        if (is_array()) return std::get<ArrayType>(value_).empty();
        else if (is_object()) return std::get<ObjectType>(value_).empty();
        return false;
    }
    bool contains(const std::string& key) const { // 返回 JSON 对象是否为空
